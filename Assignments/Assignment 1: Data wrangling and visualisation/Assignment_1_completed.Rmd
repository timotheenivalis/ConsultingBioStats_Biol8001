...
inva_cost <- read_csv("data/InvaCost_3.0.csv", guess_max = 10000)
str(inva_cost)
```
...
```{r}
 inva_cost_filt <- filter(inva_cost, 
                              Method_reliability == "High",
                              Implementation == "Observed",
                              !str_detect(Species,pattern='spp\\.'),
...
```{r}
inva_cost_filt_2 <- filter(inva_cost_filt, 
...
```{r}
costs_Aus <- inva_cost_filt_2 %>%
  filter(Official_country == "Australia")%>%
  group_by(Species)%>%
  summarise(total_cost=sum(Cost_estimate_per_year_2017_USD_exchange_rate),
            Average_cost=mean(Cost_estimate_per_year_2017_USD_exchange_rate))%>%
...
inva_cost_filt_2 %>%
  ...
  ggplot(aes(x=Species, 
             y=mean(Raw_cost_estimate_2017_USD_exchange_rate), 
             fill=Type_of_cost_merged)) +
  coord_flip() +
...
xx[,c("Species", "Raw_cost_estimate_2017_USD_exchange_rate", "Type_of_cost_merged")]
```
...
```{r}
library(readxl)
erank = read_xlsx(path = "data/expert_rankings.xlsx",
                  sheet = 'Sheet1')
...
```{r}
joined <- left_join(costs_Aus, erank)
```
...
```{r}
mjoined <- joined%>%
  mutate(BDSI_votes_2 =if_else(is.na(BDSI_votes),-1,BDSI_votes))%>%
  mutate(BIOL8001_votes_2 =if_else(is.na(BIOL8001_votes),-1,BIOL8001_votes))

ggplot(mjoined)+
  geom_point(aes(log(total_cost), BDSI_votes_2, colour = "red"))+
  geom_point(aes(log(total_cost), BIOL8001_votes_2, colour = "blue"))+
  scale_colour_manual(labels = c("BDSI_votes_2", "BIOL8001"), 
                      values =  c("red", "blue"))
...
(2 points)
```{r}
ala <- read_csv("data/ALA_records.csv")

valist <- read_xlsx("data/Various Australian lists.xlsx")
cdata <- left_join(left_join(mjoined, ala), valist)


```

```{r}
ggplot(cdata%>%filter(!is.na(occurrenceCount)), 
       aes(log(occurrenceCount), log(Average_cost)))+
  geom_point()+
  geom_smooth(method = 'lm', formula = y~x)+
  labs(title = "Relationship between average cost and count number")

ggplot(cdata%>%filter(!is.na(class)), aes(class, occurrenceCount))+
  geom_bar(stat = 'identity')+
  labs(title = "Count of invasive species for each class")
```
